...
`pipeline Docker workflows` runs micro-backends for **interactive** games only. Static puzzles may be static files served by the launcher nginx build.

## Retiring standalone clones

Legacy standalone repos can be archived once parity is verified here. Each game is documented on its own terms under `src/games/{slug}/`.
