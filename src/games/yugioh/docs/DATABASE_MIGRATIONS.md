...
The CLI/database maintenance flow runs at startup when you bring the stack up (e.g. `docker compose up --build`): first **migrations** (create/update tables), then **seed** from `data/*.csv` via `CLI commands / src/seed_from_csv.py`. For manual control use `db_manager.py reset-db`, then `migrate`, then `seed` (or `reset-and-seed` for all three). With Podman: `podman compose -f pipeline Docker workflows up --build`.
...
```bash
python3 CLI commands / src/generate_cards_csv.py
python3 CLI commands / src/seed_from_csv.py
```
