...
```bash
python3 scripts/src/generate_cards_csv.py              # metadata from card_list
python3 scripts/src/generate_cards_csv.py --fetch-images   # fill all image URLs
python3 scripts/src/generate_cards_csv.py --fill-missing-images
python3 scripts/src/validate_catalog.py              # if present
```
